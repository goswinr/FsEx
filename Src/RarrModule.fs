...
module Rarr =    
    ...
    let inline getNeg index  (rarr: Rarr<'T>)= 
        ...
        if ii<0 || ii >= len then ArgumentException.RaiseBase "Rarr.GetNeg: Failed to get index %d from Rarr of %d items: %A" index rarr.Count rarr
    ...
    let inline setNeg index value  (rarr: Rarr<'T>)= 
        let len = rarr.Count
        let ii =  if index < 0 then len + index else index
        if ii<0 || ii >= len then ArgumentException.RaiseBase "Rarr.SetNeg: Failed to set index %d to %A rom Rarr of %d items: %A" index value rarr.Count rarr
    ...
    let inline setLooped index value  (rarr: Rarr<'T>) = 
        let len = rarr.Count
        if len=0 then ArgumentException.RaiseBase "Rarr.SetLooped: Failed to Set index %d to %A in Rarr of 0 items" index value
        let t = index % len
        let ii = if t >= 0 then t  else t + len 
        rarr.List.[ii] <- value // access List directly to not check index twice  
    ...
    let inline pop  (rarr: Rarr<'T>)  =
        if rarr.Count=0 then ArgumentException.RaiseBase "Failed to pop from empty Rarr"
        let i = rarr.Count - 1        
    ...
    let inline secondLast  (rarr: Rarr<'T>)= 
        if rarr.Count < 2 then  IndexOutOfRangeException.Raise "Rarr.secondLast: Failed to get second last item of %s"  rarr.ToNiceString
        rarr.List.[rarr.Count - 2]

    /// Gets the third last item in the Rarr.
    /// Same as   this.[this.Count - 3]
    let inline thirdLast  (rarr: Rarr<'T>)= 
        if rarr.Count < 3 then  IndexOutOfRangeException.Raise "Rarr.thirdLast: Failed to get third last item of %s" rarr.ToNiceString
    ...
    let inline second  (rarr: Rarr<'T>)= 
        if rarr.Count < 2 then IndexOutOfRangeException.Raise  "Rarr.second: Failed to get second item of %s" rarr.ToNiceString
        rarr.List.[1]

    /// Gets the third item in the Rarr.
    /// Same as   this.[2]
    let inline third  (rarr: Rarr<'T>)= 
        if rarr.Count < 3 then IndexOutOfRangeException.Raise "Rarr.third: Failed to get third item of %s" rarr.ToNiceString
        rarr.List.[2]
    ...
    let inline rotate k (rarr: Rarr<'T>)  =  
        ...
        for i = 0 to r.Count-1 do 
            r.Add <| rarr.List.[negIdxLooped (i-k) rarr.Count]
    ...
    let allPairs (rarr1: Rarr<'T>) (rarr2: Rarr<'U>) :Rarr<'T*'U> =
        ...
        for i = 0 to rarr1.Count-1 do
            for j = 0 to rarr2.Count-1 do
                res.Add (rarr1.List.[i], rarr2.List.[j])
        res
    ...
    let inline private countByImpl (comparer: IEqualityComparer<'SafeKey>) (projection: 'T->'SafeKey) (getKey: 'SafeKey->'Key) (rarr: Rarr<'T>) =
    ...
    let except (itemsToExclude: seq<_>) (rarr: Rarr<'T>) : Rarr<'T> =        
    ...
    let findBack (predicate:'T->bool) (rarr: Rarr<'T>) =
        let rec loop i =             
            if i < 0 then  
                ...
                if predicate rarr.List.[i] then rarr.List.[i]  else loop (i-1)
        loop (rarr.Count-1)
    ...
    let findIndexBack (predicate:'T->bool) (rarr: Rarr<'T>) =         
        let rec go n = 
            if n < 0 then 
                KeyNotFoundException.Raise "Rarr.findIndexBack: not found in Rarr of %d items %A" rarr.Count rarr
            elif predicate rarr.List.[n] then
                n 
            else go (n-1)
        go (rarr.Count-1)
    ...
    let fold<'T, 'State> (folder: 'State -> 'T -> 'State) (state: 'State) (rarr: Rarr<'T>) =
        ...
        for i = 0 to rarr.Count-1 do 
            state <- f.Invoke(state, rarr.List.[i])
        state
    ...
    let groupBy (projection:'T -> 'Key) (rarr:Rarr<'T>) :  Rarr<'Key * Rarr<'T>> =
        if typeof<'Key>.IsValueType            
            // We avoid wrapping a StructBox, because under 64 JIT we get some "hard" tailcalls which affect performance
            then groupByImpl HashIdentity.Structural<'Key> projection id rarr
                
            // Wrap a StructBox around all keys in case the key type is itself a type using null as a representation
            else groupByImpl StructBox<'Key>.Comparer (fun t -> StructBox (projection t)) (fun sb -> sb.Value) rarr     
    ...
    let mapFold<'T, 'State, 'Result> (mapping: 'State -> 'T -> 'Result * 'State) state (rarr:Rarr<'T>) : Rarr<'Result> * 'State =
        ...
        | len ->
            ...
            let res = Rarr(len)
            for i = 0 to len-1 do
                let h, s = f.Invoke(acc, rarr.List.[i])
                res.Add h
                acc <- s
            res, acc
    ...
    let mapFoldBack<'T, 'State, 'Result> (mapping: 'T -> 'State -> 'Result * 'State) (rarr:Rarr<'T>) state : Rarr<'Result> * 'State=
        ...
        | len ->
            ...
            for i = len - 1 downto 0 do
                let h, s = f.Invoke(rarr.List.[i], acc)
                res.[i] <- h
                acc <- s
            res, acc        
    ...
    let mapi (mapping: int -> 'T -> 'U) (rarr: Rarr<'T>) : Rarr<'U> =
        ...
        let res = Rarr(rarr.Count) 
        for i = 0 to rarr.Count-1 do 
            res.Add <|  f.Invoke(i, rarr.List.[i])
        res        
    ...
    let reduce (reduction:'T -> 'T -> 'T) (rarr: Rarr<'T>) = 
        if rarr.Count = 0 then ArgumentException.RaiseBase "Rarr.reduce: The input Rarr is empty."        
        let f = OptimizedClosures.FSharpFunc<_, _, _>.Adapt(reduction)
        let mutable res = rarr.List.[0]
        for i = 1 to rarr.Count - 1 do
            res <- f.Invoke(res, rarr.List.[i])
        res
    ...
    let reduceBack (reduction:'T -> 'T -> 'T) (rarr: Rarr<'T>) = 
        if rarr.Count = 0 then ArgumentException.RaiseBase "Rarr.reduceBack: The input Rarr is empty."
        let f = OptimizedClosures.FSharpFunc<_, _, _>.Adapt(reduction)
    ...
    let skipWhile (predicate:'T->bool) (rarr: Rarr<'T>) =        
        let mutable i = 0            
        while i < rarr.Count && predicate rarr.List.[i] do 
            i <- i + 1        
        if i = rarr.Count then
            Rarr()
        else
    ...
    let takeWhile (predicate:'T->bool) (rarr: Rarr<'T>) = 
        if rarr.Count = 0 then 
            ...
            let mutable count = 0
            while count < rarr.Count && predicate rarr.List.[count] do
                count <- count + 1
            rarr.GetRange(0,count)
    ...
    let tryFindBack (predicate:'T->bool) (rarr: Rarr<'T>) :option<'T> =
        let rec loop i = 
            ...
            if predicate rarr.List.[i] then Some rarr.List.[i]  else loop (i-1)
        loop ((rarr.Count-1)) 
    ...
    let tryFindIndexBack (predicate:'T->bool) (rarr: Rarr<'T>) : option<int>=
        let rec loop i = 
            if i < 0 then None else 
            if predicate rarr.List.[i] then Some i  else loop (i-1)
        loop ((rarr.Count-1))
    ...
    module Parallel =
        ...
        let collect (mapping: 'T -> Rarr<'U>)  (rarr: Rarr<'T>) : Rarr<'U>=
            ...
            Parallel.For(0, inputLength, 
                (fun i -> result.[i] <- mapping rarr.List.[i])) |> ignore
            concat result
        ...
        let iter (action:'T->unit) (rarr: Rarr<'T>) =
            Parallel.For (0, rarr.Count, fun i -> action rarr.List.[i]) |> ignore  
        ...
        let iteri (action:int->'T->unit) (rarr: Rarr<'T>) =
            let f = OptimizedClosures.FSharpFunc<_, _, _>.Adapt(action)
            Parallel.For (0, rarr.Count, fun i -> f.Invoke(i, rarr.List.[i])) |> ignore        
        ...
        let map (mapping: 'T -> 'U) (rarr: Rarr<'T>) : Rarr<'U>=
            ...
            Parallel.For(0, inputLength, fun i ->
                result.[i] <- mapping rarr.List.[i]) |> ignore
            result
        ...
        let mapi (mapping:int-> 'T -> 'U) (rarr: Rarr<'T>) =
            ...
            let result = create inputLength  Unchecked.defaultof<_>
            Parallel.For(0, inputLength, fun i ->
                result.[i] <- f.Invoke (i, rarr.List.[i])) |> ignore
            result
        ...
        let partition (predicate:'T->bool) (rarr: Rarr<'T>) =
            ...
            for i = 0 to isTrue.Length-1 do
                if isTrue.[i] then
                    res1.Add rarr.List.[i]                    
                else
                    res2.Add rarr.List.[i]                
